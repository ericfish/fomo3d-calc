	...
	function getCurrentRoundInfo(){
		...
		tokenContract.getCurrentRoundInfo(function(error, result){
		    if(!error){
				    let tKeys = Number(web3.fromWei(result[2])).toFixed(2);
				    // console.log(tKeys);
				    $('#tkeypec').html(tKeys);
	...
	function getBuyPrice(){
		tokenContract.getBuyPrice(function(error, result){
		    if(!error){
					$('#currP').html(Number(web3.fromWei(result)).toFixed(8)); // 当前key价格
  ...
  function getPlayerInfoByAddress(){
	    ...
	    if(!error){
		    ...
		    $('#ethpec').html(Number(web3.fromWei(result[6].toNumber())).toFixed(6)); // 投入ETH
  ...
  function fomo3d() {
    var ethpec = parseFloat(document.getElementById('ethpec').innerHTML);
    var cethpec = parseFloat(document.getElementById('cethpec').innerHTML);
